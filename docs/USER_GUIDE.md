...
Swarm предоставляет MCP-сервер, который регистрирует инструмент `run_swarm` в Roo Code. После настройки вы сможете запускать Swarm прямо из чата с Roo Code.
...
### 3.3. Инструмент `run_swarm`

После подключения MCP-сервера вам станет доступен инструмент `run_swarm` со следующими параметрами:
...
| `project_files` | `integer` | ❌ | `0` | Примерное количество файлов в проекте |

Параметр `complexity` подсказывает системе, насколько задача сложная:

- `auto` — система сама определит по ключевым словам
- `low` — простые задачи: тесты, документация, форматирование (без ревью, экономный режим)
- `medium` — обычные задачи: фичи, API, багфиксы
- `high` — сложные: архитектура, рефакторинг, безопасность
- `critical` — критически важные: security, auth, production-код

### 3.4. Примеры вызовов

#### Пример 1: Простая задача (калькулятор)

```
Задача: "Напиши простой калькулятор на Python с поддержкой +, -, *, /"
```

Что произойдёт:
1. Система определит сложность как `medium` (по умолчанию)
2. Архитектор предложит структуру: функция `calculate()`, обработка ошибок
3. Кодер напишет реализацию с `operator` module
4. Ревьюер проверит на деление на ноль и корректность типов
5. Результат: готовый код калькулятора

#### Пример 2: Рефакторинг (средняя сложность)

```
Задача: "Оптимизируй API слой: добавь кэширование, валидацию запросов и обработку ошибок"
```

Что произойдёт:
1. Система обнаружит ключевые слова `optimize`, `API`, `error handling` → сложность `high`
2. Архитектор составит план рефакторинга
3. Кодер реализует изменения
4. Ревьюер проверит, что все паттерны соблюдены

#### Пример 3: Критическая задача (аутентификация)
