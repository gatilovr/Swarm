...
# Запуск роя
result = runner.run("Напиши функцию на Python для быстрой сортировки (quicksort)")
...
### Agent Orchestration (опционально)

`agent-orchestration` — это дополнительный MCP-сервер для распределённой работы AI-агентов. Он обеспечивает:

- **Очередь задач** — создание и распределение задач между агентами
- **Общую память** — обмен контекстом между агентами через key-value хранилище
- **Координацию** — блокировки ресурсов и синхронизация работы
- **Мониторинг** — отслеживание статуса и событий всех агентов
...
Сервер будет доступен через MCP-протокол и автоматически подключится к Roo Code после настройки `.roo/mcp.json`.
...
MCP-сервер уже настроен в [`.roo/mcp.json`](.roo/mcp.json) как `code-swarm`.
